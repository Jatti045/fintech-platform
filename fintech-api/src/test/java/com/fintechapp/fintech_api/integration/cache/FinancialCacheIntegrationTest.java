 ...
 * database.
 *
 * <p>These tests are skipped when Redis is unreachable so ordinary test runs
 * never require Redis — the application falls back to PostgreSQL in that case
 * (covered by {@code FinancialCacheRedisUnavailableIntegrationTest}).</p>
 *
 * <p>Cache-hit evidence: the underlying {@code TransactionRepository} sum
 * aggregation (the summary's database work) is counted via a spy.</p>
 */
class FinancialCacheIntegrationTest extends BaseIntegrationTest {
    ...
    void monthlyIncomeUpdate_invalidatesCachedSummaryRegion() throws Exception {
        ...
        mockMvc.perform(patch("/api/users/me/monthly-income")
                        .header(authHeaderName(), authHeader(user))
                        .contentType(json())
                        .content(asJson(new UpdateMonthlyIncomePayload(4200.0, currentMonthIndex(), currentYear()))))
                .andExpect(status().isOk());
    ...
    void recurringPayments_missThenHit_thenInvalidatedByTransactionCreation() throws Exception {
        ...
        for (int i = 1; i <= 4; i++) {
            createTransaction(user, null, "Rent",
                    utcToday.withDayOfMonth(1).minusMonths(i).atStartOfDay().toInstant(ZoneOffset.UTC),
